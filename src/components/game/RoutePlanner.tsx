...
export default function RoutePlanner() {
  const { planes, assignRoute, autoDispatchEnabled, autoTicketPrice, autoDispatchStrategy, setAutoDispatch, setAutoTicketPrice, setAutoDispatchStrategy } = useGameStore();
  ...
  return (
    ...
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-bold">Route Control Center v3.4</h1>
      ...
      <div className="bg-white p-4 rounded-xl border border-gray-100 flex flex-wrap gap-4 items-center">
        ...
        <div className="flex items-center gap-2 text-sm">
          <span>Strategy</span>
          <select value={autoDispatchStrategy} onChange={(e) => setAutoDispatchStrategy(e.target.value as "nearest" | "profit")} className="p-2 border rounded">
            <option value="profit">Balanced/Profit</option>
            <option value="nearest">Nearest Route</option>
          </select>
        </div>
      </div>
